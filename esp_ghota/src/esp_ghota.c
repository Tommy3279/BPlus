...
typedef struct ghota_client_handle_t
{
    ...
    struct
    {
        ...
        char browser_download_url[CONFIG_MAX_URL_LEN];
        uint8_t flags;
    } result;
    struct
    {
        char name[CONFIG_MAX_FILENAME_LEN];
        char url[CONFIG_MAX_URL_LEN]; // API URL
        char browser_download_url[CONFIG_MAX_URL_LEN]; // Download URL
        // Thêm cờ để theo dõi các trường đã tìm thấy cho asset hiện tại
        bool got_name;
        bool got_url;
        bool got_browser_url;
    } scratch_asset;
...
static void process_and_clear_scratch_asset(ghota_client_handle_t *handle) {
    // Hàm helper để xử lý scratch_asset khi kết thúc object asset
    if (handle->scratch_asset.got_name && handle->scratch_asset.got_browser_url) {
        ESP_LOGW(TAG, "--- Asset Object End: Processing Scratch ---");
        ESP_LOGW(TAG, "Pattern (filenamematch): [%s]", handle->config.filenamematch);
        ESP_LOGW(TAG, "Name    (scratch.name) : [%s]", handle->scratch_asset.name);
        ESP_LOG_BUFFER_HEXDUMP(TAG, handle->config.filenamematch, strlen(handle->config.filenamematch), ESP_LOG_WARN);
        ESP_LOG_BUFFER_HEXDUMP(TAG, handle->scratch_asset.name, strlen(handle->scratch_asset.name), ESP_LOG_WARN);

        int match_result = fnmatch(handle->config.filenamematch, handle->scratch_asset.name, 0);
        ESP_LOGW(TAG, "fnmatch result: %d (0 = match, !0 = no match)", match_result);

        if (!GetFlag(handle, GHOTA_RELEASE_VALID_ASSET) && match_result == 0) {
            // Match firmware
            strncpy(handle->result.name, handle->scratch_asset.name, CONFIG_MAX_FILENAME_LEN);
            strncpy(handle->result.url, handle->scratch_asset.url, CONFIG_MAX_URL_LEN); // Lưu API URL nếu cần
            strncpy(handle->result.browser_download_url, handle->scratch_asset.browser_download_url, CONFIG_MAX_URL_LEN); // Lưu Download URL
            ESP_LOGI(TAG, "Valid Firmware Found: %s - Download URL: %s", handle->result.name, handle->result.browser_download_url);
            SetFlag(handle, GHOTA_RELEASE_VALID_ASSET);
        } else if (!GetFlag(handle, GHOTA_RELEASE_GOT_STORAGE) && strlen(handle->config.storagenamematch) > 0 && fnmatch(handle->config.storagenamematch, handle->scratch_asset.name, 0) == 0) {
             // Match storage (vẫn dùng API URL cho storage theo logic cũ)
             strncpy(handle->result.storageurl, handle->scratch_asset.url, CONFIG_MAX_URL_LEN);
             ESP_LOGI(TAG, "Valid Storage Asset Found: %s - API URL: %s", handle->scratch_asset.name, handle->result.storageurl);
             SetFlag(handle, GHOTA_RELEASE_GOT_STORAGE);
        } else {
             ESP_LOGW(TAG, "Asset '%s' did NOT match any required pattern.", handle->scratch_asset.name);
        }
    } else {
        if(handle->scratch_asset.got_name) { // Log nếu có tên mà thiếu url tải
             ESP_LOGW(TAG, "Asset '%s' ended without required browser_download_url.", handle->scratch_asset.name);
        }
    }

    // Luôn xóa scratch sau khi xử lý
    memset(&handle->scratch_asset, 0, sizeof(handle->scratch_asset));
}
...
static void lwjson_callback(lwjson_stream_parser_t *jsp, lwjson_stream_type_t type)
{
    ...
    //ESP_LOGI(TAG, "Callback: Type=%d, StackPos=%d, TopType=%d", type, jsp->stack_pos, (jsp->stack_pos > 0) ? jsp->stack[jsp->stack_pos - 1].type : -1);
    ...
    if (!GetFlag(handle, GHOTA_RELEASE_GOT_TAG))
    {
            ...
            && strcasecmp(jsp->stack[1].meta.name, "tag_name") == 0)
        {
            ...
            ESP_LOGI(TAG, "Got tag_name: %s", handle->result.tag_name);
        }
    }
    // --- Logic xử lý assets ---
    // Xác định xem có đang ở trong một object asset không (mức 5, bắt nguồn từ key "assets" ở mức 2/3)
    bool in_asset_object = (jsp->stack_pos == 5 &&
    ...
    // Bắt đầu một asset object? (Không cần làm gì đặc biệt ở đây với logic mới)

    // Đang đọc một KEY bên trong asset object?
    if (type == LWJSON_STREAM_TYPE_KEY && in_asset_object) {
         const char* current_key = jsp->stack[4].meta.name;
         //ESP_LOGE(TAG, "@@@ Asset Key Found: [%s] @ StackPos=%d", current_key, jsp->stack_pos);
         // Xóa scratch nếu đây là key "name" và scratch cũ chưa được xóa (bắt đầu asset mới)
         if (strcasecmp(current_key, "name") == 0 && handle->scratch_asset.got_name) {
             //ESP_LOGE(TAG, "@@@ New asset detected ('name' key), clearing previous scratch asset info.");
             memset(&handle->scratch_asset, 0, sizeof(handle->scratch_asset));
         }
    }
    // Đang đọc một VALUE (STRING) bên trong asset object?
    else if (type == LWJSON_STREAM_TYPE_STRING && in_asset_object && jsp->stack[4].type == LWJSON_STREAM_TYPE_KEY) {
        const char* current_key = jsp->stack[4].meta.name;
        const char* current_value = jsp->data.str.buff;
        //ESP_LOGE(TAG, "@@@ Asset Key: [%s], Value: [%s] @ StackPos=%d", current_key, current_value, jsp->stack_pos);

        if (strcasecmp(current_key, "name") == 0) {
            strncpy(handle->scratch_asset.name, current_value, CONFIG_MAX_FILENAME_LEN);
            handle->scratch_asset.got_name = true;
            //ESP_LOGE(TAG, "@@@ --> Stored scratch_asset.name = %s", handle->scratch_asset.name);
        } else if (strcasecmp(current_key, "url") == 0) {
            strncpy(handle->scratch_asset.url, current_value, CONFIG_MAX_URL_LEN);
            handle->scratch_asset.got_url = true;
            //ESP_LOGE(TAG, "@@@ --> Stored scratch_asset.url (API URL)");
        } else if (strcasecmp(current_key, "browser_download_url") == 0) {
            // !!! LOG QUAN TRỌNG !!!
            //ESP_LOGE(TAG, "@@@ FOUND 'browser_download_url' KEY !!!");
            strncpy(handle->scratch_asset.browser_download_url, current_value, CONFIG_MAX_URL_LEN);
            handle->scratch_asset.got_browser_url = true;
            //ESP_LOGE(TAG, "@@@ --> Stored scratch_asset.browser_download_url = %s", handle->scratch_asset.browser_download_url);
        }
    }
    // Kết thúc một asset object (quay lại mức 4)?
    // Lưu ý: Stack pos có thể là 3 hoặc 4 tùy vào cách lwjson xử lý khi kết thúc object trong array
    else if (type == LWJSON_STREAM_TYPE_OBJECT_END && (jsp->stack_pos == 3) && jsp->stack[1].type == LWJSON_STREAM_TYPE_KEY && strcasecmp(jsp->stack[1].meta.name, "assets") == 0) {
        //ESP_LOGE(TAG,"@@@ End of Asset Object detected @ StackPos=%d", jsp->stack_pos);
         // Gọi hàm xử lý scratch data cho asset vừa kết thúc
         process_and_clear_scratch_asset(handle);
    }
     // Kết thúc mảng assets
     else if (type == LWJSON_STREAM_TYPE_ARRAY_END && jsp->stack_pos == 2 && jsp->stack[1].type == LWJSON_STREAM_TYPE_KEY && strcasecmp(jsp->stack[1].meta.name, "assets") == 0) {
        //ESP_LOGE(TAG,"@@@ End of Assets Array detected @ StackPos=%d", jsp->stack_pos);
         // Kiểm tra xem có asset hợp lệ nào được tìm thấy không (log này có thể giữ lại từ hàm process...)
         if (!GetFlag(handle, GHOTA_RELEASE_VALID_ASSET)) { ESP_LOGW(TAG, "Finished parsing assets array, No Valid Firmware Assets Found!"); }
         if (!GetFlag(handle, GHOTA_RELEASE_GOT_STORAGE) && strlen(handle->config.storagenamematch) > 0) { ESP_LOGW(TAG, "Finished parsing assets array, No Valid Storage Assets Found!"); }
     }
     // Thêm log cho các trường hợp khác nếu cần để debug sâu hơn
     // else {
     //     ESP_LOGD(TAG, "Other callback: Type=%d, StackPos=%d", type, jsp->stack_pos);
     // }
} // End function
...
esp_err_t ghota_check(ghota_client_handle_t *handle)
{
    ...
    {
        ESP_LOGI(TAG, "Github Release API Returned: %d", esp_http_client_get_status_code(client));
        if (GetFlag(handle, GHOTA_RELEASE_VALID_ASSET))
        {
            ...
            ESP_LOGI(TAG, "Current Version %u.%u.%u", handle->current_version.major, handle->current_version.minor, handle->current_version.patch); // Sửa %d thành %u
            ESP_LOGI(TAG, "New Version %u.%u.%u", handle->latest_version.major, handle->latest_version.minor, handle->latest_version.patch); // Sửa %d thành %u
...
esp_err_t ghota_update(ghota_client_handle_t *handle)
{
    ...
    esp_http_client_config_t httpconfig = {
        // <<< THAY ĐỔI DÒNG NÀY >>>
        // .url = handle->result.url, // URL API cũ
        .url = handle->result.browser_download_url, // <<< SỬ DỤNG URL MỚI NÀY >>>
        .crt_bundle_attach = esp_crt_bundle_attach,
        .keep_alive_enable = true,
        .buffer_size_tx = 4096, // Giữ nguyên hoặc điều chỉnh nếu cần
    ...
    esp_https_ota_config_t ota_config = {
        ...
        .http_client_init_cb = http_client_set_header_cb, // Header Accept vẫn có thể cần thiết
        // .bulk_flash_erase = true, // Tùy chọn: Xóa toàn bộ trước khi ghi OTA (có thể nhanh hơn)
        // .partial_http_download = true, // Tùy chọn: Cho phép tải từng phần nếu kết nối bị gián đoạn
        // .max_http_request_size = 2048 // Tùy chọn: Tăng kích thước request nếu cần
    };

    esp_https_ota_handle_t https_ota_handle = NULL;
    esp_err_t err = esp_https_ota_begin(&ota_config, &https_ota_handle); // Gọi với config mới
    if (err != ESP_OK)
    {
...
esp_err_t ghota_start_update_task(ghota_client_handle_t *handle)
{
    ...
    if (state == eDeleted || state == eInvalid)
    {
        ...
        if (xTaskCreate(ghota_task, "ghota_task", 16384, handle, 5, &handle->task_handle) != pdPASS)
        {
