...
typedef enum {
    ...
    EVENT_TYPE_PHONE_VALIDATED, // Sự kiện xác thực số điện thoại (có thể từ keypad hoặc từ server)
    ...
    EVENT_TYPE_DOOR_STATUS_CHANGED, // Trạng thái cửa thay đổi (do bo điều khiển upload)
...
// Cấu trúc cho sự kiện phản hồi từ bo điều khiển
typedef struct {
    uint8_t board_address;
    uint8_t command_word;
    uint8_t status; // 0x00: Success, 0xFF: Fail [cite: 3]
    union {
        struct {
            uint8_t channel_number;
            uint8_t door_state; // 0x00: Open, 0x01: Closed, 0xFF: Fail [cite: 18]
        } single_door_status;
        struct {
            uint8_t total_channels;
            uint8_t door_states[64]; // Giả sử tối đa 64 kênh, điều chỉnh nếu cần [cite: 1]
        } all_door_status;
        // Thêm các cấu trúc dữ liệu phản hồi khác nếu cần
    } data;
...
typedef union {
    ...
    uint8_t lock_channel_to_open;                   // Số kênh cần mở khóa (dùng cho lệnh mở khóa)
    lock_controller_response_t lock_response;       // Phản hồi từ bo điều khiển khóa
